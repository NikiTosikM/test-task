...
from src.app.repositories.mappers import (
    ContractMapper,
    LeadMapper,
    OperatorMapper,
...
class ContractRepository(BaseRepository, ContractMapper):
    ...
    def get_distribution_stats(
        self, source_id: UUID, hours_back: int = 24
    ) -> dict[UUID, float]:
        """
...
class LeadRepository(BaseRepository, LeadMapper):
    ...
    def load_contracts(self, lead_id: UUID) -> list[LeadsContractSchema]:
        query = (
            select(
                Contract.source_id,
                Contract.operator_id)
            .select_from(self.model)
            .join(Contract, self.model.id == Contract.lead_id)
        ...
        return self.data_mapper.contract_conversion(lead_id, result)
